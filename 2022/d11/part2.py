...
class Monkey:
    def __init__(self, string):
        ...
        self.target = lambda new: target_true if new % self.test == 0 else target_false
...
class Game:
    def __init__(self, input, iterations=20):
        ...
        self.worryfree = prod(monkey.test for monkey in self.monkeys)

    def play(self):
        for _ in range(self.iterations):
            for monkey in self.monkeys:
                for target, item in monkey.throw_items():
                    self.monkeys[target].items.append(item % self.worryfree)


g = Game(open("input"), 10000)
g.play()
