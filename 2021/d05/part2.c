...
int main()
{
    ...
    for (int i; i < amount; i++)
    {
        ...
        else if ((cur->x1 - cur->x2) * (cur->y1 - cur->y2) > 0)
        { /* / */
            for (int x = min(cur->x1, cur->x2), y = min(cur->y1, cur->y2); x <= max(cur->x1, cur->x2); x++, y++)
            {
                grid[x][y]++;
                overlap += grid[x][y] == 2 ? 1 : 0;
            }
        }
        else
        { /* \ */
            for (int x = min(cur->x1, cur->x2), y = max(cur->y1, cur->y2); x <= max(cur->x1, cur->x2); x++, y--)
            {
                grid[x][y]++;
                overlap += grid[x][y] == 2 ? 1 : 0;
            }
